...
@section('page')
الفواتير التى تم ارسالها من الشركات الى {{ auth()->user()->details->company_name }}
@endsection
...
<table id="html5-extension" class="table table-hover non-hover" style="width:100%">
    ...
    <tbody>
        ...
        @if($invoice['issuerId'] !== auth()->user()->details->company_id && $invoice['receiverId'] === auth()->user()->details->company_id)
        <tr>
            ...
            <td>
                <form action="{{ route('RejectDocument',$invoice['uuid']) }}" method="post">
                    @csrf
                    @method('PUT')
                    <button class="btn btn-danger" type="submit" onclick="return confirm('هل انت متأكد من  رفض الفاتورة؟');">رفض الفاتورة</button>
                </form>
            </td>
        </tr>
        @endif
