...
@section('page')
الفواتيرالتى تم تقديم طلب لإلغائها من خلال الشركات
@endsection
...
<table id="html5-extension" class="table table-hover non-hover" style="width:100%">
    ...
    <tbody>
        ...
        @if($invoice['receiverId'] == auth()->user()->details->company_id && $invoice['cancelRequestDate']!==null &&
        $invoice['status'] === 'Valid' && $invoice['declineCancelRequestDate']===null)
        <tr>
            ...
            <td>صحيحة</td>
            @endif
            <td>{{Carbon\Carbon::parse($invoice['cancelRequestDelayedDate'])->format('d-m-Y')}}</td>
            <td>{{Carbon\Carbon::parse($invoice['cancelRequestDelayedDate'])->format('m-i-s')}}</td>
            ...
            <td>
                <form action="{{ route('declineCancellDocument',$invoice['uuid']) }}" method="post">
                    @csrf
                    @method('PUT')
                    <button class="btn btn-danger" type="submit" onclick="return confirm('هل انت متأكد من الغاء الإلغاء للفاتورة؟');">إلغاء الإلغاء للفاتورة</button>
                </form>
            </td>
        </tr>
        @endif
